...
def practisingByGrade(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for b in range(6, 13):
        for i in range(string.__len__() - 1):
            if int(termNumber) == 1:
                list[b - 1][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, term1=string[i],
                    student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so).count()
            elif int(termNumber) == 2:
                list[b - 1][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, term2=string[i],
                    student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so).count()
            else:
                list[b - 1][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, year=string[i],
                    student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so).count()

        list[b - 1][0] = TBNam.objects.filter(year_id__time=yearNumber, student_id__classes__block_id__number=b,
            year_id__school_id__upper_organization=so).count()
        for i in range(string.__len__() - 1):
            if list[b - 1][0] != 0:
...
def practisingByMajor(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for b in range(10, 13):
        for (k, ban) in enumerate(BAN_CHOICE):
            for i in range(string.__len__() - 1):
                if int(termNumber) == 1:
                    list[b - 1][k][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, term1=string[i],
                        student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so,
                        student_id__ban_dk=ban[0]).count()
                elif int(termNumber) == 2:
                    list[b - 1][k][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, term2=string[i],
                        student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so,
                        student_id__ban_dk=ban[0]).count()
                else:
                    list[b - 1][k][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, year=string[i],
                        student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so,
                        student_id__ban_dk=ban[0]).count()

            list[b - 1][k][0] = TBNam.objects.filter(year_id__time=yearNumber, student_id__classes__block_id__number=b,
                year_id__school_id__upper_organization=so, student_id__ban_dk=ban[0]).count()
...
def practisingByDistrict(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for (k, s) in enumerate(listSchool):
        ...
        if ((k < numberSchool - 1) and (listSchool[k].district != listSchool[k + 1].district)) or (
        k == numberSchool - 1):
            for i in range(string.__len__() - 1):
                if tong[0] > 0:
...
def learningByGrade(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for b in range(6, 13):
        for i in range(string.__len__() - 1):
            if int(termNumber) < 3:
                list[b - 1][2 * i + 1] += TBHocKy.objects.filter(term_id__year_id__time=yearNumber,
                    term_id__number=termNumber, hl_hk=string[i], student_id__classes__block_id__number=b,
                    term_id__year_id__school_id__upper_organization=so).count()
            else:
                list[b - 1][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, hl_nam=string[i],
                    student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so).count()
...
def learningByMajor(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for b in range(10, 13):
        for (k, ban) in enumerate(BAN_CHOICE):
            for i in range(string.__len__() - 1):
                if int(termNumber) < 3:
                    list[b - 1][k][2 * i + 1] += TBHocKy.objects.filter(term_id__year_id__time=yearNumber,
                        term_id__number=termNumber, hl_hk=string[i], student_id__classes__block_id__number=b,
                        term_id__year_id__school_id__upper_organization=so, student_id__ban_dk=ban[0]).count()
                else:
                    list[b - 1][k][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, hl_nam=string[i],
                        student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so,
                        student_id__ban_dk=ban[0]).count()
...
def titleByGrade(request, yearNumber=None, termNumber=-1, isExcel=0):
    ...
    for b in range(6, 13):
        for i in range(string.__len__() - 1):
            if int(termNumber) < 3:
                list[b - 1][2 * i + 1] += TBHocKy.objects.filter(term_id__year_id__time=yearNumber,
                    term_id__number=termNumber, danh_hieu_hk=string[i], student_id__classes__block_id__number=b,
                    term_id__year_id__school_id__upper_organization=so).count()
            else:
                list[b - 1][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, danh_hieu_nam=string[i],
                    student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so).count()
        ...
        for (k, ban) in enumerate(BAN_CHOICE):
            for i in range(string.__len__() - 1):
                if int(termNumber) < 3:
                    ...
                    list[b - 1][k][2 * i + 1] += TBNam.objects.filter(year_id__time=yearNumber, danh_hieu_nam=string[i],
                        student_id__classes__block_id__number=b, year_id__school_id__upper_organization=so,
                        student_id__ban_dk=ban[0]).count()
...
def countFinalMark(request, type=0, yearNumber=None, termNumber=-1, subjectIndex=-1, blockIndex=-1, isExcel=0):
    ...
    if blockIndex != None:
        ...
        for l in listSchool:
            ...
            if len(term) > 0:
                t = term[0]
                sum = Mark.objects.filter(term_id=t, subject_id__type=subjectType,
                    subject_id__class_id__block_id__number=blockIndex, current=True).count()
            else:
                sum = 0
            sumsum += sum
            ...
            for i in range(numberLevel):
                if type == '0':
                    ...
                    if len(term) > 0:
                        t = term[0]
                        slList[i] = Mark.objects.filter(term_id=t, subject_id__type=subjectType,
                            subject_id__class_id__block_id__number=blockIndex, ck__lt=level[i], ck__gt=level[i + 1],
                            current=True).count()
                    ...
                    if int(termNumber) < 3:
                        ...
                        if len(term) > 0:
                            t = term[0]
                            slList[i] = Mark.objects.filter(term_id=t, subject_id__type=subjectType,
                                subject_id__class_id__block_id__number=blockIndex, tb__lt=level[i], tb__gt=level[i + 1],
                                current=True).count()
                            if sum > 0: ptList[i] = round(slList[i] * 100.0 / sum, 2)
                    else:
                        ...
                        if len(year) > 0:
                            y = year[0]
                            slList[i] = TKMon.objects.filter(subject_id__type=subjectType,
                                subject_id__class_id__year_id=y, subject_id__class_id__block_id__number=blockIndex,
                                tb_nam__lt=level[i], tb_nam__gt=level[i + 1], current=True).count()
                            print slList[i]
                            if sum > 0: ptList[i] = round(slList[i] * 100.0 / sum, 2)
