...
def getMark(subjectChoice, selectedTerm):
    ...
    if selectedTerm.number == 1:
        markList = Mark.objects.filter(term_id=selectedTerm.id, subject_id=subjectChoice, current=True).order_by(
        ...
        markList = Mark.objects.filter(term_id=selectedTerm.id, subject_id=subjectChoice, current=True).order_by(
            'student_id__index', 'student_id__first_name', 'student_id__last_name', 'student_id__birthday')
        tbhk1List = Mark.objects.filter(term_id=beforeTerm, subject_id=subjectChoice, current=True).order_by(
            'student_id__index', 'student_id__first_name', 'student_id__last_name', 'student_id__birthday')
        tbnamList = TKMon.objects.filter(subject_id=subjectChoice, current=True).order_by('student_id__index',
...
def markTable(request, term_id=-1, class_id=-1, subject_id=-1, move=None):
    ...
    selectedClass = None
    if classChoice != -1:
        subjectList = Subject.objects.filter(class_id=classChoice, primary__in=[0, selectedTerm.number, 3, 4]).order_by(
            "index", 'name')
...
def markForTeacher(request, type=1, term_id=-1, subject_id=-1, move=None):
    ...
    termList = Term.objects.filter(year_id=yearChoice, number__lt=3).order_by('number')
    if int(type) == 1:
        subjectList = Subject.objects.filter(teacher_id=idTeacher, class_id__year_id=yearChoice,
            primary__in=[0, selectedTerm.number, 3, 4]).order_by("class_id__block_id__number", "index")
    else:
        teaching_class = request.user.teacher.current_homeroom_class()
        subjectList = Subject.objects.filter(class_id=teaching_class,
            primary__in=[0, selectedTerm.number, 3, 4]).order_by("index")
        if subjectChoice != -1:
...
def capNhapMienGiam(request, class_id, student_id):
    ...
    subjectList = Subject.objects.filter(class_id=class_id,
        name__in=['GDQP-AN', u'Thể dục', u'Âm nhạc', u'Mĩ thuật', 'GDQP']).order_by('index', 'name')
    term1Mark = Mark.objects.filter(subject_id__class_id=class_id, student_id=student_id, term_id__number=1,
        subject_id__name__in=['GDQP-AN', u'Thể dục', u'Âm nhạc', u'Mĩ thuật', 'GDQP']).order_by('subject_id__index',
        'subject_id__name')
    term2Mark = Mark.objects.filter(subject_id__class_id=class_id, student_id=student_id, term_id__number=2,
        subject_id__name__in=['GDQP-AN', u'Thể dục', u'Âm nhạc', u'Mĩ thuật', 'GDQP']).order_by('subject_id__index',
        'subject_id__name')
    tkMonList = TKMon.objects.filter(subject_id__class_id=class_id, student_id=student_id,
        subject_id__name__in=['GDQP-AN', u'Thể dục', u'Âm nhạc', u'Mĩ thuật', 'GDQP']).order_by('subject_id__index',
        'subject_id__name')
