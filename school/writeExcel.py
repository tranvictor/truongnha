...
def printASubject(class_id, termNumber, s, sub, x, y, ls, number):
    ...
    monList = Mark.objects.filter(student_id__classes=class_id, subject_id=sub, term_id=selected_term).order_by(
...
def printPage20(class_id, termNumber, s, length, subjectList):
    ...
    tbhkList = TBHocKy.objects.filter(student_id__classes=class_id, term_id=selected_term,
...
def printPage31(class_id, s, tbNamList, x, y):
    ...
    s.write_merge(x + 1, x + 2, y + 8, y + 10, u'Xếp loại lại về HK, HL, sau KT lại các môn học hoặc rèn luyện về HK',
        h10)
    ...
    s.write_merge(x + 14, x + 17, y + 12, y + 12,
        '-Được lên lớp sau khi kiểm tra lại các môn học hoặc rèn luyện trong hè: ' + str(llSauHe), h81)
...
def printFirstPage(class_id, book):
    ...
    if (selectedClass.block_id.number <= 9) & (selectedClass.block_id.number >= 6):
        ...
        s.write_merge(x + 52, x + 52, y + 0, y + 6,
            'Tỉnh/TP:.................................................................................', f3)
        ...
        s.write_merge(x + 54, x + 54, y + 4, y + 6, yearString, f3)
        s.write_merge(x + 58, x + 58, y, y + 6,
            'Các môn học tự chọn nâng cao(nếu là ban Cơ bản):......................................................................'
            , f3)
...
def printPage2(class_id, book):
    ...
    s.col(
        8).width = A3_WIDTH - STT_WIDTH - LASTNAME_WIDTH - FIRSTNAME_WIDTH - BIRTHDAY_WIDTH - PLACE_WIDTH - SEX_WIDTH - DAN_TOC_WIDTH - UU_TIEN_WIDTH
    ...
    s.write_merge(x + 1, x + 4, y + 7, y + 7,
        u'Con LS, con TB, con BB,\n con của người được hưởng\n chế độ như TB, con GĐ\n có công với CM ', h10)
...
def printPage15(book, class_id, number, mon1, mon2, mon3):
    ...
    s.write_merge(max + 5, max + 5, 0, 11, u'Trong trang này có......... điểm được sửa chữa,' +
                                           u' trong đó môn: ' + mon1 + u'....... điểm, ' + mon2 + u'.......điểm, ' + mon3 + u'.......điểm'
        , h8)
...
def exportMark(request, term_id, subject_id, colMieng=4, col15Phut=4, colMotTiet=4):
    ...
    if selectedTerm.number == 1:
        ...
        hk1List = Mark.objects.filter(subject_id=subject_id, term_id__number=1, current=True).order_by(
            'student_id__index', 'student_id__first_name', 'student_id__last_name', 'student_id__birthday')
        cnList = TKMon.objects.filter(subject_id=subject_id, current=True).order_by('student_id__index',
...
# in phieu bao diem
def printMarkToExcel(termNumber, selectedClass, checkNxList, s, pupilList, markList, tkMonList, ddHKList, tbHKList,
                     TBNamList, ddHK1List=None):
    ...
    numberLine = 50
    if termNumber == 1:
        yearString = 'Học kỳ I - ' + 'Năm học ' + str(selectedClass.year_id.time) + '-' + str(
            selectedClass.year_id.time + 1)
    else:
        yearString = 'Học kỳ II - ' + 'Năm học ' + str(selectedClass.year_id.time) + '-' + str(
            selectedClass.year_id.time + 1)
    ...
    for dd, tbhk, tbNam in zip(ddHKList, tbHKList, TBNamList):
        ...
        for t in range(1, 6):
            s.write_merge(x + i * numberLine + 12 + j + t, x + i * numberLine + 12 + j + t, y, y + 3,
                '......................................................................', h8)
            s.write_merge(x + i * numberLine + 12 + j + t, x + i * numberLine + 12 + j + t, y + 5, y + 8,
                '.....................................................................', h8)
        ...
        s.write_merge(x + i * numberLine + 17 + j + t, x + i * numberLine + 17 + j + t, y + 6, y + 8,
            'Giáo viên chủ nhiệm', h8)
...
def markForClass(termNumber, class_id):
    ...
    for sub in subjectList:
        l = Mark.objects.filter(subject_id=sub.id, term_id__number=termNumber, current=True).order_by(
            'student_id__index', 'student_id__first_name', 'student_id__last_name', 'student_id__birthday')
        markList.append(l)
        if termNumber == 2:
            tkMon = TKMon.objects.filter(subject_id=sub.id, current=True).order_by('student_id__index',
                'student_id__first_name', 'student_id__last_name', 'student_id__birthday')
            tkMonList.append(tkMon)
        if sub.nx:  checkNxList += [1]
        else:  checkNxList += [0]
    ddHKList = TKDiemDanh.objects.filter(student_id__classes=class_id, term_id=selected_term,
        student_id__attend__is_member=True).order_by('student_id__index', 'student_id__first_name',
        'student_id__last_name', 'student_id__birthday').distinct()
    tbHKList = TBHocKy.objects.filter(student_id__classes=class_id, term_id=selected_term,
        student_id__attend__is_member=True).order_by('student_id__index', 'student_id__first_name',
    ...
    printMarkToExcel(termNumber, selectedClass, checkNxList, s, pupilList, markList, tkMonList, ddHKList, tbHKList,
        TBNamList, ddHK1List)
...
def markExcelForAStudent(request, class_id, student_id, term_id):
    ...
    if selectedTerm.number == 1:
        subjectList = Subject.objects.filter(class_id=class_id, primary__in=[0, selectedTerm.number, 3, 4]).order_by(
            "index", 'name')
    else:
        subjectList = Subject.objects.filter(class_id=class_id).order_by("index", 'name')
    ...
    ddHKList = TKDiemDanh.objects.filter(student_id__classes=class_id, term_id=term_id, student_id=student_id,
        student_id__attend__is_member=True).distinct()
    tbHKList = TBHocKy.objects.filter(student_id__classes=class_id, term_id=term_id, student_id=student_id,
        student_id__attend__is_member=True).distinct()
    TBNamList = TBNam  .objects.filter(year_id=selected_year, student_id__classes=class_id,
        student_id=student_id).distinct()

    ddHK1List = None
    if selectedTerm.number == 2:
        term1 = Term.objects.get(year_id=selected_year, number=1)
        ddHK1List = TKDiemDanh.objects.filter(student_id__classes=class_id, term_id=term1, student_id=student_id,
            student_id__attend__is_member=True).distinct()

    setSizeOfMarkClass(s, len(pupilList))
    printMarkToExcel(selectedTerm.number, selectedClass, checkNxList, s, pupilList, markList, tkMonList, ddHKList,
        tbHKList, TBNamList, ddHK1List)
