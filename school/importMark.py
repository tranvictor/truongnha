...
def excelToArray(s,x,y,colMieng,col15Phut,colMotTiet,arrMark,arrTime,timeNow,timeToEdit,position,isNx,diffMessage):
            ...
            for i in range(x,s.nrows):
                ...
                for p,m in list:
                    if (p.last_name==lastName) & (p.first_name==firstName) & (p.birthday.strftime('%d/%m/%Y')== birthday):
                        ...
                        editMarkMessage= excelToArray(s,i,y+4,colMieng,col15Phut,colMotTiet,arrMark,arrTime,timeNow,timeToEdit,position,isNx,diffMessage)
                        if editMarkMessage!="": break;

                        editMarkMessage,m.ck,arrTime[3*MAX_COL+1]=process(s,i,y+colMieng+col15Phut+colMotTiet+4,m.ck,arrTime[3*MAX_COL+1],timeNow,timeToEdit,position,isNx,diffMessage)

                        if editMarkMessage !='': break
                        if isNx:
                            if selectedTerm.number==1:
                                editMarkMessage,m.tb,arrTime[3*MAX_COL+2]=process(s,i,y+colMieng+col15Phut+colMotTiet+5,m.tb,arrTime[3*MAX_COL+2],timeNow,timeToEdit,position,isNx,diffMessage)
                            else:
                                editMarkMessage,m.tb,arrTime[3*MAX_COL+2]=process(s,i,y+colMieng+col15Phut+colMotTiet+6,m.tb,arrTime[3*MAX_COL+2],timeNow,timeToEdit,position,isNx,diffMessage)
                                for p,tkMon in zip(pupilList,tkMonList):
                                    if (p.last_name==lastName) & (p.first_name==firstName) & (p.birthday.strftime('%d/%m/%Y')== birthday):
                                        editMarkMessage,tkMon.tb_nam,tempTime=process(s,i,y+colMieng+col15Phut+colMotTiet+7,tkMon.tb_nam,tkMon.time,timeNow,timeToEdit,position,isNx,diffMessage)
