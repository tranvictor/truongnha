...
def finish(request, active_term=0, term_number=None, year_number=None,is_calculate=0):
    ...
    if (term_number != None) & (is_calculate == '1'):
        if int(term_number) < 3:
            term = Term.objects.get(year_id__school_id=request.user.userprofile.organization, year_id__time=year_number, number=term_number)
    ...
    # get term 1
    finish_learning1,not_finish_learning1,finish_practising1,not_finish_practising1,finish_all1,not_finish_all1 = countDetailTerm(term1.id)
    number_finish_learning1 = number_pupil - TBHocKy.objects.filter(term_id = term1, student_id__disable = False, hl_hk = None).count()
    number_finish_practising1 = number_pupil - TBNam.objects.filter(year_id = current_year, student_id__disable = False, term1 = None).count()
    number_finish_title1 = number_pupil - TBHocKy.objects.filter(term_id = term1,student_id__disable = False,danh_hieu_hk=None).count()

    list.append((number_finish_learning1,number_finish_practising1,number_finish_title1,not_finish_learning1,not_finish_practising1,not_finish_all1))

    # get term 2
    finish_learning2,not_finish_learning2,finish_practising2,not_finish_practising2,finish_all2,not_finish_all2 = countDetailTerm(term2.id)
    number_finish_learning2 = number_pupil - TBHocKy.objects.filter(term_id = term2, student_id__disable = False, hl_hk = None).count()
    number_finish_practising2 = number_pupil - TBNam.objects.filter(year_id = current_year, student_id__disable = False, term2 = None).count()
    number_finish_title2 = number_pupil - TBHocKy.objects.filter(term_id = term2,student_id__disable = False,danh_hieu_hk=None).count()
    list.append((number_finish_learning2,number_finish_practising2,number_finish_title2,not_finish_learning2,not_finish_practising2,not_finish_all2))
    #get year
    finish_learning3,not_finish_learning3,finish_practising3,not_finish_practising3,finish_all3,not_finish_all3 = countDetailYear(current_year.id)
    number_finish_learning3 = number_pupil - TBNam.objects.filter(year_id = current_year, student_id__disable = False, hl_nam = None).count()
    number_finish_practising3 = number_pupil - TBNam.objects.filter(year_id = current_year, student_id__disable = False, year = None).count()
    number_finish_title3 = number_pupil - TBNam.objects.filter(year_id = current_year, student_id__disable = False,danh_hieu_nam=None).count()
...
def calculateTKMon(class_id):
    ...
    mgList = Mark.objects.filter(subject_id__class_id=class_id,mg=True,current=True).order_by('student_id__index','student_id__first_name','student_id__last_name','student_id__birthday','subject_id__index','subject_id__name')

    for m in mgList:
        anotherMark =Mark.objects.get(subject_id=m.subject_id,student_id = m.student_id,term_id__number =3-m.term_id.number)
