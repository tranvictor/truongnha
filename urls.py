...
urlpatterns = patterns('',
    ...
    url(r'^login/(?P<redirect_after>[/%a-zA-Z0-9_]+)/$', app_views.login, name= "login_redirect"),
    url(r'^logout/$', 'django.contrib.auth.views.logout',
            {'template_name': os.path.join(settings.TEMPLATE_DIRS[0], 'app/logout.html'), 'next_page': '/'}, name = "logout"),
    url(r'^register/$', app_views.register, name = 'register'),
    url(r'^manageRegister/$', app_views.manage_register, name='manage_register'),
    url(r'^manageRegister/(?P<sort_by_date>\w+)/$', app_views.manage_register, name="manage_register"),
    url(r'^manageRegister/(?P<sort_by_date>\w+)/(?P<sort_by_status>\w+)/$', app_views.manage_register, name="manage_register"),
